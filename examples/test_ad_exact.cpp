/**
   ...
   USES THE EXACT 2D-4D ADAPTIVE SPACE
...
#define AD_PLAN_EPS 2
#define AD_TRACK_EPS 1
#define AD_PLAN_TIMELIMIT 10.0
#define AD_TRACK_TIMELIMIT 20.0
#define TUNNEL_HALFWIDTH 0.30
//#define TUNNEL_HALFWIDTH 0.05
//#define TUNNEL_HALFWIDTH 0.65
#define F3D_EPS 1.5
...
// @avemula CHANGED corrected the radius of the new sphere
//#define RADIUS 0.4301
#define RADIUS 1.0
...
int planxythetatimelat_ad(int argc, char *argv[], vector<SBPL_DynamicObstacle_t> dynObs) {
  ...
  EnvironmentNAVXYTHETATIMELATADEXACT *environment_navxythetatimelatadexact;

  // Initializing environment and planner
  environment_navxythetatimelatadexact = new EnvironmentNAVXYTHETATIMELATADEXACT();
  ...
  environment_navxythetatimelatadexact->Computedxy();

  if(argc == 4 || argc == 8)
    {
      printf("using mprims from [%s]...\n", argv[2]); fflush(stdout);
      if(!environment_navxythetatimelatadexact->InitializeEnv(argv[1], perimeterptsV, argv[2]))
	{
  ...
  else
    {
      ...
      if(!environment_navxythetatimelatadexact->InitializeEnv(argv[1], perimeterptsV, NULL))
	{
	  printf("ERROR: InitializeEnv failed\n");
	  throw new SBPL_Exception();
	}
  ...
  if(!environment_navxythetatimelatadexact->setDynamicObstacles(dynObs, false)){
    printf("The start state is in collision!\nSolution does not exist\n\n");
    throw new SBPL_Exception();
  }

  if(!environment_navxythetatimelatadexact->InitializeMDPCfg(&MDPCfg))
    {
      printf("ERROR: InitializeMDPCfg failed\n");
      throw new SBPL_Exception();
    }

  // @avemula ADDED a new function to set the time bound
  //environment_navxythetatimelatadexact->setTimeBound();

  // @avemula DEBUG forcing 3d planning
  //environment_navxythetatimelatadexact->FORCE3DPLANNING = true;

  printf("done\n"); fflush(stdout);

  printf("Getting environment params..."); fflush(stdout);
  vector<SBPL_xythetatime_mprimitive> motionprimitiveV;
  environment_navxythetatimelatadexact->GetEnvParms(&size_x, &size_y, &startx_d, &starty_d, &starttheta_d, &starttime_d, &goalx_d, &goaly_d, &goaltheta_d, &cellsize, &timeResolution, &nominalvel_d, &timetoturn45degsinplace_d, &obsthresh_uc, &dynobsthresh_uc, &motionprimitiveV);
  printf("done!\n"); fflush(stdout);
  ...
  fflush(stdout);
  planner = new AdaptivePlanner(environment_navxythetatimelatadexact, bforwardsearch);
  ...
  environment_navxythetatimelatadexact->visualizeState(MDPCfg.startstateid, 120, "Start state");
  #endif
  ...
  #ifdef VISUALIZATIONS_ENABLED
  environment_navxythetatimelatadexact->visualizeState(MDPCfg.goalstateid, 240, "Goal state");
  #endif
  ...
  int bRet;
  elapsed_time = clock() - start_t;
  //environment_navxythetatimelatadexact->visualizeEnvironment();
  ...
  //bRet = planner->replan(300.0, &solution_stateIDs_V);
  bRet = planner->replan(300.0, allocated_time_secs, &solution_stateIDs_V);
  ...
  /*for (int i=0; i<solution_stateIDs_V.size(); i++) {
    ...
    EnvNAVXYTHETATIMELATHashEntry_t * s = environment_navxythetatimelatadexact->getEntryFromID(solution_stateIDs_V[i]);
    printf("%d, %d, %d, %d, %d\n", s->X, s->Y, s->Theta, s->T, s->TempT);
    
    }*/

  environment_navxythetatimelatadexact->PrintTimeStat(stdout);
  
  vector<SBPL_4Dpt_t> xythetatimePath;
  environment_navxythetatimelatadexact->ConvertStateIDPathintoXYThetaPath(&solution_stateIDs_V, &xythetatimePath);
  ...
  planner->saveStats("adaptive_run.stats");
  
  printf("\n");
  fflush(NULL);

  delete environment_navxythetatimelatadexact;
  return bRet;
  
}

// @avemula ASK WHAT IS BASELINE?
int planxythetatimelat(int argc, char *argv[], vector<SBPL_DynamicObstacle_t> dynObs) {
  ...
  fflush(stdout);
  planner = new AdaptivePlanner(environment_navxythetatimelatadexact, bforwardsearch);
  planner->set_initialsolution_eps_plan(ad_plan_eps);
  planner->set_new_sphere_radius(RADIUS);
  planner->set_tunnel_width(TUNNEL_HALFWIDTH);
  printf("done!\n");
  fflush(stdout);
  ...
  planner->saveStats("full4d_run.stats");
...
int main(int argc, char *argv[]) {
  ...
  // Full 4D planning
  planxythetatimelat(argc, argv, dynObs);
