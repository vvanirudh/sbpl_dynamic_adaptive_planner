/**
   ...
   Implements the interfaces for the (x,y,theta,time) adaptive space (EXACT version) 2D - 4D
...
#ifndef __ENVIRONMENT_NAVXYTHETATIMELATAD_EXACT_H_
#define __ENVIRONMENT_NAVXYTHETATIMELATAD_EXACT_H_
...
class EnvironmentNAVXYTHETATIMELATADEXACT : public EnvironmentNAVXYTHETATIMELAT_AD
{
 ...
 public:
  ...
  std::vector<tSphere_exact> spheres;
  ...
  ~EnvironmentNAVXYTHETATIMELATADEXACT();
  EnvironmentNAVXYTHETATIMELATADEXACT();
  ...
  // 3D expansions - Useless in the exact version since it is 2D - 4D
  ...
  void Get3DSuccs_allT(int SourceStateID, vector<int> *SuccIDV, vector<int> *CostV, vector<EnvNAVXYTHETATIMELATAction_t*> *actionV=NULL);
  ...
  // 4D expansions - Should be changed slightly
  ...
  // 2D expansions - Should be implemented in the right way
  ...
  // @avemula ADDED grow all spheres function - NOT NEEDED in this case
  void growAllSpheres();
  ...
  void visualizeSphere(tSphere_exact* sphere, std::string name);
  ...
  EnvNAVXYTHETATIMELATHashEntry_t* CreateNewHashEntry_hash(int X, int Y, int Theta, int T, float TempT);
  EnvNAVXYTHETATIMELATHashEntry_t* GetHashEntry_lookup(int X, int Y, int Theta, int T);
  EnvNAVXYTHETATIMELATHashEntry_t* CreateNewHashEntry_lookup(int X, int Y, int Theta, int T, float TempT);

  //pointers to functions
  EnvNAVXYTHETATIMELATHashEntry_t* (EnvironmentNAVXYTHETATIMELATADEXACT::*GetHashEntry)(int X, int Y, int Theta, int T);
  EnvNAVXYTHETATIMELATHashEntry_t* (EnvironmentNAVXYTHETATIMELATADEXACT::*CreateNewHashEntry)(int X, int Y, int Theta, int T, float TempT);

  int GetStateFromCoord(int x, int y, int theta, int t);
  int GetStateFromCoord(int x, int y, int theta, int t, float tempT);
  ...
  inline bool isHighDPath(std::vector<int> *stateIDV){
    for(unsigned int i = 0; i < stateIDV->size(); i++){
      ...
      if(HashEntry->Theta >= INVALID_THETA && HashEntry->T ==INVALID_TIME && stateIDV->at(i)!=EnvNAVXYTHETATIMELAT.goalstateid){
	return false;
      }
  ...
  // @avemula ADDED a new function to check if the given state is goal or not
  bool isGoal(int stateID);

  // @avemula ADDED a new function to set the time bound
  void setTimeBound();

  // @avemula ADDED a helper print path function
  void printStatePath(vector<int> * StateIDV);
};
